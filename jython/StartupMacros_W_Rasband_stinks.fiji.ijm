...
function flip(string, idx) {
  n = lengthOf(string);
  a = substring(string, 0, idx);
  mid = substring(string, idx, idx + 1);
  b = substring(string, idx + 1, n);
...
function toggle(channel){
	Stack.getActiveChannels(mask);
	new_mask = flip(mask, channel - 1);
	Stack.setActiveChannels(new_mask);
}

macro "Toggle Channel 1 [1]" {toggle(1);} 
macro "Toggle Channel 2 [2]" {toggle(2);} 
macro "Toggle Channel 3 [3]" {toggle(3);} 
macro "Toggle Channel 4 [4]" {toggle(4);} 
macro "Toggle Channel 5 [5]" {toggle(5);} 
macro "Toggle Channel 6 [6]" {toggle(6);} 
macro "Toggle Channel 7 [7]" {toggle(7);} 


macro "color [g]" {Stack.setDisplayMode("grayscale");} 
