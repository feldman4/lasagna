...
datasets = '20170212_96W-G075',
file_pattern = lasagna.io.default_file_pattern_new

channels = 'DAPI', 'FITC', 'Cy3', 'TxRed', 'Cy5'
...
register_sequencing_to = 2

known_offsets = {'5B1': ((0, 0), (2, 0), (0, 0), (0, 0), (1, 0))}


def load_pL4X_files():
    dataset = datasets[0]
    df_files_all = paths()
    filt = df_files_all['dataset'] == dataset
    df_files_all = df_files_all[filt]

    exp = lasagna.conditions_.Experiment(worksheet=dataset, g_file='Lasagna Supreme')
    filt_pL4_ctrls = exp.ind_vars_table['cells'].apply(lambda s: 'pL5' not in s)

    import warnings
    with warnings.catch_warnings():
        warnings.simplefilter('ignore')
        df_files = df_files_all.set_index('well')[filt_pL4_ctrls].reset_index()
    return df_files


def offset_channels(name):
    for tag, offsets in known_offsets.items():
        if tag in name:
            return offsets
    return None
...
def do_alignment(df_files):
    """Aligns 20170204_96W-G074 data. 
    
    Within each sequencing cycle, all channels except DAPI are aligned to Cy3.
    DO and sequencing cycles are aligned using DAPI.
    """
    ...
    for well, df_ in df_files.groupby('well'):
        ...
        for f in files:
            ...
            if 'DO' in f:
                ...
                # register sequencing channels
                offsets = offset_channels(f)
                
                if offsets is None:
                    data_reg = [data_[register_sequencing_to]] + list(data_[1:])
                    offsets = [o.astype(int) for o in register_images(data_reg)]
