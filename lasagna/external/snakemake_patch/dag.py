...
class DAG:
    ...
    def init(self, progress=False):
        ...
        try:
            ...
            if not snakemake.ignore_missing:
                raise AttributeError # flag not set
            else:
                # split input rules to get one targetjob per input per rule
                targetjobs = []
                for rule in self.targetrules:
                    for i in range(len(rule.input)):
                        input_backup = rule.input
                        mini_input =  Namedlist(rule.input[i:i+1])
                        rule._input = mini_input
                        targetjob = self.rule2job(rule)
                        targetjobs.append(targetjob)
                        # restore
                        rule._input = input_backup
                for job in targetjobs:
                    try:
                        job2 = self.update([job], progress=progress)
                        # print('adding target job', job2)
                        # print()
                        self.targetjobs.add(job2)
                    except AssertionError:
                        # ignore targetjob
                        continue

                    else:
                        targetjobs = map(self.rule2job, self.targetrules)
    ...
    def update(self, jobs, file=None, visited=None, skip_until_dynamic=False, progress=False):
        ...
        for job in jobs:
            ...
            try:
                ...
                try:
                    import snakemake
                    if snakemake.ignore_missing:
                        # print('update got MIE from', job, 'out of', jobs)
                        assert False
                except AttributeError:
                    pass
    ...
    def update_(self, job, visited=None, skip_until_dynamic=False, progress=False):
        ...
        if missing_input:
            # DF MOD
            # print('hung up on', job)
            # print()
            # print('with input', job.input)
            # print()
            # print('with missing_input', missing_input)
            # self.delete_job(job, recursive=False)  # delete job from tree
            self.delete_job(job, recursive=True)  # delete the shit out of job
