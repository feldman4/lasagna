...
@contextmanager
def queue_callback(queue, instance, *props):
    """Place callbacks in queue, with format (callback, args, kwargs)

    This is a context manager. Within the context block, no callbacks
    will be issued. Callbacks are appended to provided queue.
    ...
    Example::
        ...
        def mouse_wrapper(nuclei):
            def f(mouse_event):
                global todo
                j, i = mouse_event.getX(), mouse_event.getY()
                with queue_callback(todo, image, 'selection'):
                    image.selection = nuclei[i, j]
            return f

        # may run on separate interpreter
        while 1:
            if todo:
                f, args, kwargs = todo.pop()
                f(*args, **kwargs)
    ...
    for old, prop in zip(vals, props):
        ...
        new = p.__get__(instance)
        if old != new:
            queue += [[p.notify, (instance, old, new), {}]]
